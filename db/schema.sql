...
CREATE TABLE IF NOT EXISTS projects (
  ...
  tech_stack    TEXT,                         -- display string ('Next.js · Prisma · …')
  sort_order    INTEGER NOT NULL DEFAULT 0
);
...
CREATE TABLE IF NOT EXISTS region_tones (
  id            INTEGER PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS region_deeds (
  id            INTEGER PRIMARY KEY,
  region_id     INTEGER NOT NULL REFERENCES regions(id) ON DELETE CASCADE,
  body          TEXT NOT NULL,
  sort_order    INTEGER NOT NULL DEFAULT 0
...
CREATE TABLE IF NOT EXISTS lost_pages (
...
CREATE TABLE IF NOT EXISTS voice_lines (
  ...
  -- fly-over trigger geometry (kind='region' lines; NULL for journey events)
  map_u         REAL CHECK (map_u IS NULL OR map_u BETWEEN 0 AND 1),
  map_v         REAL CHECK (map_v IS NULL OR map_v BETWEEN 0 AND 1),
...
-- ─────────────────── 3D CHARACTERS DISPLAYED IN SCROLLS ─────────────────────

-- Character models shown inside region scrolls alongside the information.
-- model_url NULL = character defined but model not yet uploaded (UI shows a
-- framed placeholder with the caption until the GLB lands under /public/models).
CREATE TABLE IF NOT EXISTS characters (
  id            INTEGER PRIMARY KEY,
  region_id     INTEGER REFERENCES regions(id) ON DELETE CASCADE,  -- NULL = global
  slug          TEXT NOT NULL UNIQUE,         -- 'gandalf', 'smaug'
  name          TEXT NOT NULL,
  caption       TEXT,                         -- lore line shown under the model
  model_url     TEXT,                         -- '/models/gandalf.glb'
...
-- Login rate limiting (keyed by lowercased email and by IP)
...
CREATE INDEX IF NOT EXISTS idx_profile_langs_profile   ON profile_languages(profile_id);
CREATE INDEX IF NOT EXISTS idx_skills_category         ON skills(category_id);
CREATE INDEX IF NOT EXISTS idx_exp_highlights_exp      ON experience_highlights(experience_id);
CREATE INDEX IF NOT EXISTS idx_exp_skills_skill        ON experience_skills(skill_id);
CREATE INDEX IF NOT EXISTS idx_edu_degrees_edu         ON education_degrees(education_id);
CREATE INDEX IF NOT EXISTS idx_edu_courses_edu         ON education_courses(education_id);
CREATE INDEX IF NOT EXISTS idx_proj_highlights_proj    ON project_highlights(project_id);
CREATE INDEX IF NOT EXISTS idx_proj_skills_skill       ON project_skills(skill_id);
CREATE INDEX IF NOT EXISTS idx_region_tones_region     ON region_tones(region_id);
CREATE INDEX IF NOT EXISTS idx_region_deeds_region     ON region_deeds(region_id);
CREATE INDEX IF NOT EXISTS idx_region_links_region     ON region_links(region_id);
CREATE INDEX IF NOT EXISTS idx_voice_lines_region      ON voice_lines(region_id);
...
CREATE VIEW IF NOT EXISTS v_region_scroll AS
SELECT
  r.id            AS region_id,
  r.slug,
  r.place,
